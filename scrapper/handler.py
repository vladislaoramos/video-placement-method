...
def write_desc_to_file(features):
    ...
    """
    logger.debug("adding descriptions")
...
class DataHandler:
    ...
    def write_desc(self):
        for item in self.raw_data:
            ...
            logger.debug("adding description about video #" + video_id)
            video_desc = [
                # video_id
            ...
            logger.debug("write descriptions.tsv")
    ...
    def write_video_stats(self):
        ...
        for key, stat in metrics.items():
            logger.debug("updating [%d] metrics", key)
            for item in self.raw_data:
                video_features = item[0]
                video_id = video_features.get("id")
                logger.debug("checking whether video #[%d] is new", video_id)
                if self.first_day:
                    logger.debug("video #[%d] is new so adding its [%d] metrics", video_id, key)
                    stat.append([video_id, video_features.get("statistics").get(key)])
                else:
                    logger.debug("video #[%d] is in history so try to append its [%d] metrics", video_id, key)
                    for video in stat:
                        if video[0] == video_id:
                            logger.debug("append metrics because video #[%d] has no n-days history", video_id)
